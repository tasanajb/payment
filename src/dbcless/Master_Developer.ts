...
interface IMaster_DeveloperMethod {
  load(request: Request, id: string | IMaster_Developer): Promise<void>;
...
export class Master_Developer implements IMaster_Developer, IMaster_DeveloperMethod {
  ...
  constructor(request?: Request | IMaster_Developer, developer_code?: string) {
    if (developer_code) {
      ...
      if (request instanceof Request) {
        throw new Error("developer_code is required.");
      }
  ...
  async load(request: Request, developer_code: string): Promise<void> {
      const item = await Master_Developer.findOne(request, { developer_code: developer_code });
